...
void AVLTree<K, V>::rebalance(Node*& subtree)
{
    ...
    if (balance > 1) {
        ...
        if (height(subtree->left->left) >= height(subtree->left->right))
            rotateRight(subtree);       // Case 1: left-left
        else
            rotateLeftRight(subtree);   // Case 2: left-right
    } else if (balance < -1) {
        // Right-heavy
        if (height(subtree->right->right) >= height(subtree->right->left))
            rotateLeft(subtree);        // Case 3: right-right
        else
            rotateRightLeft(subtree);   // Case 4: right-left
    } else {
        // Case 5: already balanced, just update height
...
void AVLTree<K, V>::remove(Node*& subtree, const K& key)
{
    ...
    if (key < subtree->key) {
        ...
        if (subtree->left == NULL && subtree->right == NULL) {
             ...
             * Use the in-order predecessor (rightmost node of left subtree).
             */
