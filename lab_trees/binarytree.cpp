/**
 * @file binarytree.cpp
 * Definitions of the binary tree functions you'll be writing for this lab.
 * You'll need to modify this file, as well as binarytree.h.
 */

#include <iostream>

using namespace std;

/**
 * @return The height of the binary tree. Recall that the height of a binary
 *  tree is just the length of the longest path from the root to a leaf, and
 *  that the height of an empty tree is -1.
 */
template <typename T>
int BinaryTree<T>::height() const
{
    // Call recursive helper function on root
    return height(root);
}

/**
 * Private helper function for the public height function, with an additional
 * Node* parameter to allow for recursive calls. NOTE: use this as an example
 * for writing your own private helper functions.
 * @param subRoot
 * @return The height of the subtree.
 */
template <typename T>
int BinaryTree<T>::height(const Node* subRoot) const
{
    // Base case
    if (subRoot == NULL)
        return -1;

    // Recursive definition
    return 1 + max(height(subRoot->left), height(subRoot->right));
}

/**
 * Prints out the values of the nodes of a binary tree in order.
 * That is, everything to the left of a node will be printed out before that
 * node itself, and everything to the right of a node will be printed out after
 * that node.
 */
template <typename T>
void BinaryTree<T>::printLeftToRight() const
{
    // Call recursive helper function on root
    printLeftToRight(root);

    // Do not remove this line - used for correct print output
    cout << endl;
}

/**
 * Private helper function for printLeftToRight.
 * Performs in-order traversal (left, root, right).
 * @param subRoot The current node in the recursion.
 */
template <typename T>
void BinaryTree<T>::printLeftToRight(const Node* subRoot) const
{
    // Base case
    if (subRoot == NULL)
        return;

    // Recursively print left subtree
    printLeftToRight(subRoot->left);

    // Print current node
    cout << subRoot->elem << " ";

    // Recursively print right subtree
    printLeftToRight(subRoot->right);
}

/**
 * Flips the tree over a vertical axis, modifying the tree itself
 * (i.e. not creating a flipped copy).
 */
template <typename T>
void BinaryTree<T>::mirror()
{
    // Call recursive helper function on root
    mirror(root);
}

/**
 * Private helper function for mirror.
 * Swaps left and right children recursively.
 * @param subRoot The current node in the recursion.
 */
template <typename T>
void BinaryTree<T>::mirror(Node* subRoot)
{
    // Base case
    if (subRoot == NULL)
        return;

    // Swap the left and right children
    Node* temp = subRoot->left;
    subRoot->left = subRoot->right;
    subRoot->right = temp;

    // Recursively mirror the subtrees
    mirror(subRoot->left);
    mirror(subRoot->right);
}

/**
 * Prints out all the possible paths from the root of the tree to any leaf node.
 * That is, all sequences starting at the root node and continuing downwards,
 * ending at a leaf node. Paths ending in a left node should be printed before
 * paths ending in a node further to the right.
 */
template <typename T>
void BinaryTree<T>::printPaths() const
{
    // Use a vector to store the current path
    vector<T> path;
    printPaths(root, path);
}

/**
 * Private helper function for printPaths.
 * Recursively finds all root-to-leaf paths.
 * @param subRoot The current node in the recursion.
 * @param path The current path from root to this node.
 */
template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, vector<T>& path) const
{
    // Base case
    if (subRoot == NULL)
        return;

    // Add current node to path
    path.push_back(subRoot->elem);

    // If this is a leaf, print the path
    if (subRoot->left == NULL && subRoot->right == NULL) {
        cout << "Path:";
        for (size_t i = 0; i < path.size(); i++) {
            cout << " " << path[i];
        }
        cout << " " << endl;
    } else {
        // Recursively explore left and right subtrees
        printPaths(subRoot->left, path);
        printPaths(subRoot->right, path);
    }

    // Backtrack: remove current node from path
    path.pop_back();
}

/**
 * Each node in a tree has a distance from the root node - the depth of that
 * node, or the number of edges along the path from that node to the root. This
 * function returns the sum of the distances of all nodes to the root node (the
 * sum of the depths of all the nodes). Your solution should take O(n) time,
 * where n is the number of nodes in the tree.
 * @return The sum of the distances of all nodes to the root.
 */
template <typename T>
int BinaryTree<T>::sumDistances() const
{
    // Call recursive helper starting at depth 0
    return sumDistances(root, 0);
}

/**
 * Private helper function for sumDistances.
 * Computes sum of depths recursively.
 * @param subRoot The current node in the recursion.
 * @param depth The current depth from root.
 * @return The sum of distances for all nodes in the subtree.
 */
template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot, int depth) const
{
    // Base case
    if (subRoot == NULL)
        return 0;

    // Return current depth plus sum of distances in subtrees
    return depth + sumDistances(subRoot->left, depth + 1) + sumDistances(subRoot->right, depth + 1);
}

/**
 * @return True if an in-order traversal of the tree would produce a
 *  nondecreasing list output values, and false otherwise. This is also the
 *  criterion for a binary tree to be a binary search tree.
 */
template <typename T>
bool BinaryTree<T>::isOrdered() const
{
    // Use a pointer to track the previous value in in-order traversal
    // Allocate on heap to persist across recursive calls
    T* prev = NULL;
    bool result = isOrdered(root, prev);
    // Clean up allocated memory
    if (prev != NULL)
        delete prev;
    return result;
}

/**
 * Private helper function for isOrdered.
 * Checks if in-order traversal produces non-decreasing sequence.
 * @param subRoot The current node in the recursion.
 * @param prev Pointer to the previous value in in-order traversal (heap-allocated).
 * @return True if the subtree is ordered.
 */
template <typename T>
bool BinaryTree<T>::isOrdered(const Node* subRoot, T*& prev) const
{
    // Base case: empty tree is ordered
    if (subRoot == NULL)
        return true;

    // Check left subtree
    if (!isOrdered(subRoot->left, prev))
        return false;

    // Check current node against previous value
    if (prev != NULL && *prev > subRoot->elem)
        return false;

    // Update prev to current element
    if (prev == NULL)
        prev = new T(subRoot->elem);
    else
        *prev = subRoot->elem;

    // Check right subtree
    return isOrdered(subRoot->right, prev);
}
