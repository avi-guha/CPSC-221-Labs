...
{
    // common[word] = number of files that contain the word
...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    ...
    for (const auto& pair : common) {
        ...
        // word must appear in every file
        if (pair.second != file_word_maps.size())
            continue;

        // word must appear >= n times in each file
        bool appears_enough = true;
        for (const auto& file_map : file_word_maps) {
            auto it = file_map.find(word);
            if (it == file_map.end() || it->second < n) {
                appears_enough = false;
                break;
            }
        }
