...
{
   // hash the characters of the word and test and compare the hash values
    Dict<char, int> wordDict(256);
    Dict<char, int> testDict(256);
    ...
    for (auto& kv : wordDict) {
        if (testDict[kv.first] != kv.second) {
            return false;
        }
    }
    for (auto& kv : testDict) {
        if (wordDict[kv.first] != kv.second) {
            return false;
        }
    }
    return true;
